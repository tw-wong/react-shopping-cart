...
const Home = () => {
  ...
  const CheckOut = () => {
    return (
      <div className="row float-right clearfix">
        <span className="font-weight-bold">Total amount:&nbsp;</span> { config.currency +' '+ totalAmount }
      </div>
    );
  }
  ...
  useEffect(() => {
    const url = 'https://my-json-server.typicode.com/tw-wong/vue-shopping-cart/products';
